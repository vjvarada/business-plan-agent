...
class ProjectStructureManager:
    ...
    def create_project(self, project_name):
        ...
        for folder, description in subfolders.items():
            ...
            readme.write_text(f'# {folder.replace("_", " ").title()}\n\n{description}\n')
...
'''
    ...
    def get_structure_summary(self):
        ...
        for folder in ['consolidated_research', 'research_archive', 'templates', 'scripts_archive']:
            folder_path = self.base_dir / folder
            if folder_path.exists():
                file_count = len(list(folder_path.rglob('*'))) - len(list(folder_path.rglob('*/')))
                summary['core_folders'][folder] = file_count
                summary['total_files'] += file_count
        
        # Find project folders (exclude core folders)
        core_folders = {'consolidated_research', 'research_archive', 'templates', 'scripts_archive', 
                       'snapshot', 'snapshot_test', '_archive'}
        
        for item in self.base_dir.iterdir():
            if item.is_dir() and item.name not in core_folders and not item.name.startswith('.'):
...
def main():
    ...
    parser.add_argument('--base-dir', default='.tmp',
                       help='Base directory (default: .tmp)')
    ...
    if args.project and not args.init_only:
        ...
        if manager.create_project(args.project):
            ...
            print(f"  2. Create config: .tmp/{args.project}/config/{args.project}_config.json")
