...
try:
    ...
    subprocess.check_call(
        [sys.executable, "-m", "pip", "install", "gspread", "gspread-formatting"]
    )
    import gspread
    from gspread_formatting import CellFormat, NumberFormat, format_cell_range
...
FORMAT_CURRENCY_K = CellFormat(
    numberFormat=NumberFormat(type="NUMBER", pattern='$#,##0,"K"')
)
FORMAT_CURRENCY_M = CellFormat(
    numberFormat=NumberFormat(type="NUMBER", pattern='$#,##0,,"M"')
)
        ...
        for i, row in enumerate(data):
            for j, cell in enumerate(row):
                if cell in ERROR_VALUES:
                    sheet_errors.append(
                        {"row": i + 1, "col": col_letter(j + 1), "error": cell}
                    )
...
def fix_balance_sheet(spreadsheet) -> bool:
    ...
    try:
        ...
        print(
            f"Found rows - Cash: {cash_row+1}, Equity: {equity_row+1}, Retained: {retained_row+1}"
        )
        ...
        if cash_row >= 0:
            ...
            if cum_cash_row >= 0:
                ...
                for col in range(3, 14):
                    ...
                    cells.append(
                        gspread.Cell(
                            cash_row + 1, col, f"='Cash Flow'!{col_l}{cum_cash_row + 1}"
                        )
                    )
                bs.update_cells(cells, value_input_option="USER_ENTERED")
                print(
                    f"  Linked Cash row {cash_row+1} to Cash Flow row {cum_cash_row+1}"
                )
        ...
        if pat_row >= 0:
            ...
            if pl_pat_row >= 0:
                ...
                for col in range(3, 14):
                    ...
                    cells.append(
                        gspread.Cell(
                            pat_row + 1, col, f"='P&L'!{col_l}{pl_pat_row + 1}"
                        )
                    )
                    ...
                    for col in range(3, 14):
                        ...
                        cells.append(
                            gspread.Cell(
                                equity_row + 1,
                                col,
                                f"='Funding Cap Table'!{col_l}{equity_raised_row + 1}",
                            )
                        )
                    cf.update_cells(cells, value_input_option="USER_ENTERED")
                    print(
                        f"  Linked Equity row {equity_row+1} to Funding Cap Table Equity Raised row {equity_raised_row+1}"
                    )
                elif seed_row >= 0:
                    cells = []
                    for col in range(3, 14):
                        ...
                        if series_a_row >= 0:
                            formula_parts.append(
                                f"'Funding Cap Table'!{col_l}{series_a_row + 1}"
                            )
                        if series_b_row >= 0:
                            formula_parts.append(
                                f"'Funding Cap Table'!{col_l}{series_b_row + 1}"
                            )
                        cells.append(
                            gspread.Cell(equity_row + 1, col, "+".join(formula_parts))
                        )
                    cf.update_cells(cells, value_input_option="USER_ENTERED")
                    print(f"  Linked Equity row {equity_row+1} to Funding Cap Table")
                else:
                    print(
                        "  Could not find funding source rows (Equity Raised / Seed / Series), skipping equity link"
                    )
            except gspread.WorksheetNotFound:
                ...
                if prev_col:
                    cells.append(
                        gspread.Cell(
                            cum_row + 1,
                            col,
                            f"={prev_col}{cum_row + 1}+{'+'.join(formula_parts)}",
                        )
                    )
                else:
                    cells.append(
                        gspread.Cell(cum_row + 1, col, f"={'+'.join(formula_parts)}")
                    )
...
def verify_links(spreadsheet) -> Dict:
    ...
    for source_sheet, target_sheet, description in expected_links:
        try:
            ...
            for row in formulas:
                for cell in row:
                    if f"'{target_sheet}'" in str(cell) or f"={target_sheet}!" in str(
                        cell
                    ):
                        has_link = True
                        break
...
def main():
    ...
    parser.add_argument(
        "--years", type=int, default=5, help="Number of years (for trim-years action)"
    )
    parser.add_argument(
        "--target-pct",
        type=float,
        default=35.0,
        help="Target SM percent (for rebalance-sm action)",
    )
