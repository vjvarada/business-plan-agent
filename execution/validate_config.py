...
class ConfigValidator:
    ...
    def validate_required_fields(self):
        ...
        if not self.config.get('company_name'):
            self.add_issue(ValidationLevel.ERROR, 'company_name',
                           "Missing company name", "Add 'company' or 'company_name'")
        ...
        if 'starting_year' not in self.config:
            ...
            if isinstance(gen, dict) and isinstance(gen.get('starting_year'), int):
                ...
                self.add_issue(ValidationLevel.WARNING, 'starting_year',
                               f'Missing; defaulting to {yr}',
                               'Set starting_year explicitly')

        for field in ['company_name', 'tax_rate', 'starting_year']:
            if field not in self.config:
                self.add_issue(ValidationLevel.ERROR, field, f"Required field '{field}' is missing")
    ...
    def validate_general_params(self):
        ...
        if tax is not None:
            if not isinstance(tax, (int, float)):
                self.add_issue(ValidationLevel.ERROR, 'tax_rate', f'Must be a number, got {type(tax).__name__}')
            elif not (0 <= tax <= 1):
                self.add_issue(ValidationLevel.ERROR, 'tax_rate', f'Must be 0-1, got {tax}', 'Use 0.25 for 25%')

        yr = self.config.get('starting_year')
        if yr is not None:
            if not isinstance(yr, int):
                self.add_issue(ValidationLevel.ERROR, 'starting_year', f'Must be int, got {type(yr).__name__}')
            elif yr < 2020 or yr > 2035:
                self.add_issue(ValidationLevel.WARNING, 'starting_year', f'{yr} seems unusual (expected 2020-2035)')
        ...
        if isinstance(gen, dict):
            for key, lo, hi, label in [
                ...
                if v is not None and isinstance(v, (int, float)):
                    if v < lo or v > hi:
                        self.add_issue(ValidationLevel.WARNING, f'general.{key}',
            ...
            for key in ['interest_rate', 'cost_inflation']:
                ...
                if v is not None and isinstance(v, (int, float)):
                    if not (0 <= v <= 1):
                        self.add_issue(ValidationLevel.ERROR, f'general.{key}',
                                       f'Must be 0-1, got {v}', 'Use 0.05 for 5%')
    ...
    def validate_revenue_streams(self):
        ...
        if not isinstance(streams, list):
            ...
            return
        if not streams:
            self.add_issue(ValidationLevel.ERROR, 'revenue_streams', 'No revenue streams defined')
            return
        if len(streams) > 10:
            self.add_issue(ValidationLevel.WARNING, 'revenue_streams',
        ...
        for i, s in enumerate(streams):
            ...
            if not isinstance(s, dict):
                ...
                continue
            if not s.get('name'):
                self.add_issue(ValidationLevel.ERROR, f'{pfx}.name', 'Missing stream name')

            # Check price (accept both 'price' and 'unit_price')
            price = s.get('price', s.get('unit_price'))
            if price is not None:
                if not isinstance(price, (int, float)):
                    self.add_issue(ValidationLevel.ERROR, f'{pfx}.price', f'Must be number, got {type(price).__name__}')
                elif price < 0:
                    self.add_issue(ValidationLevel.ERROR, f'{pfx}.price', f'Cannot be negative: {price}')
                elif price == 0:
                    self.add_issue(ValidationLevel.WARNING, f'{pfx}.price', 'Price is 0 — intentional?')

            # Growth (accept 'growth' or 'growth_rate')
            growth = s.get('growth', s.get('growth_rate'))
            if growth is not None:
                if not isinstance(growth, (int, float)):
                    self.add_issue(ValidationLevel.ERROR, f'{pfx}.growth', f'Must be number')
                elif growth < -0.5 or growth > 10:
                    self.add_issue(ValidationLevel.WARNING, f'{pfx}.growth',
            ...
            if cogs is not None:
                if not isinstance(cogs, (int, float)):
                    self.add_issue(ValidationLevel.ERROR, f'{pfx}.cogs_pct', f'Must be number')
                elif not (0 <= cogs <= 1):
                    self.add_issue(ValidationLevel.ERROR, f'{pfx}.cogs_pct',
                                   f'Must be 0-1, got {cogs}', 'Use 0.20 for 20%')

            # Volume
            vol = s.get('volume', s.get('initial_volume'))
            if vol is not None and isinstance(vol, (int, float)) and vol < 0:
                self.add_issue(ValidationLevel.ERROR, f'{pfx}.volume', f'Cannot be negative: {vol}')
    ...
    def validate_fixed_costs(self):
        ...
        if not isinstance(fc, list):
            self.add_issue(ValidationLevel.ERROR, 'fixed_costs', f'Must be list or object, got {type(fc).__name__}')
            return
        if len(fc) > 15:
            self.add_issue(ValidationLevel.WARNING, 'fixed_costs', f'{len(fc)} categories — consider consolidating')
        ...
        for i, c in enumerate(fc):
            ...
            if not c.get('name') and not c.get('category'):
                self.add_issue(ValidationLevel.ERROR, pfx, "Missing 'name' or 'category'")
            amt = c.get('annual_cost', c.get('amount', c.get('value')))
            if amt is not None:
                if not isinstance(amt, (int, float)):
                    self.add_issue(ValidationLevel.ERROR, f'{pfx}.annual_cost', f'Must be number')
                elif amt < 0:
                    self.add_issue(ValidationLevel.ERROR, f'{pfx}.annual_cost', f'Cannot be negative: {amt}')
    ...
    def validate_headcount(self):
        ...
        if isinstance(depts, list):
            if not depts:
                self.add_issue(ValidationLevel.WARNING, 'headcount.departments', 'Empty departments list')
            for i, d in enumerate(depts):
                ...
                if not isinstance(d, dict):
                    ...
                    continue
                if not d.get('name'):
                    self.add_issue(ValidationLevel.ERROR, f'{pfx}.name', 'Missing department name')
                sal = d.get('salary', 0)
                if isinstance(sal, (int, float)) and sal < 0:
                    self.add_issue(ValidationLevel.ERROR, f'{pfx}.salary', f'Cannot be negative: {sal}')
                y0 = d.get('y0_count', 0)
                if isinstance(y0, (int, float)) and y0 < 0:
                    self.add_issue(ValidationLevel.ERROR, f'{pfx}.y0_count', f'Cannot be negative: {y0}')
                gr = d.get('growth', 0)
        ...
        found_any = False
        for prefix in ['engineering', 'sales', 'ops', 'ga', 'product', 'data', 'support']:
            ...
            if sal_key in hc or y0_key in hc:
                ...
                if sal is not None and isinstance(sal, (int, float)) and sal < 0:
                    self.add_issue(ValidationLevel.ERROR, f'headcount.{sal_key}', f'Cannot be negative')
                y0 = hc.get(y0_key)
                if y0 is not None and isinstance(y0, (int, float)) and y0 < 0:
                    self.add_issue(ValidationLevel.ERROR, f'headcount.{y0_key}', f'Cannot be negative')

        if not found_any:
            self.add_issue(ValidationLevel.WARNING, 'headcount',
    ...
    def validate_funding(self):
        ...
        if not isinstance(fund, dict):
            self.add_issue(ValidationLevel.WARNING, 'funding', 'Missing or invalid funding block')
            return
        ...
        if isinstance(rounds, list):
            for i, r in enumerate(rounds):
                ...
                if not isinstance(r, dict):
                    ...
                    continue
                if not r.get('name'):
                    self.add_issue(ValidationLevel.WARNING, f'{pfx}.name', 'Missing round name')
                amt = r.get('amount')
                if amt is not None:
                    if not isinstance(amt, (int, float)):
                        self.add_issue(ValidationLevel.ERROR, f'{pfx}.amount', f'Must be number')
                    elif amt < 0:
                        self.add_issue(ValidationLevel.ERROR, f'{pfx}.amount', f'Cannot be negative')
                yr = r.get('year')
                if yr is not None and isinstance(yr, (int, float)):
                    if yr < 0 or yr > 15:
                        self.add_issue(ValidationLevel.WARNING, f'{pfx}.year',
        ...
        for key in ['seed', 'series_a', 'series_b', 'series_c']:
            if key in fund:
                ...
                if isinstance(amt, (int, float)) and amt < 0:
                    self.add_issue(ValidationLevel.ERROR, f'funding.{key}', 'Cannot be negative')
        ...
        if isinstance(old_rounds, list) and old_rounds:
            ...
            for i, r in enumerate(old_rounds):
                ...
                if amt is not None and isinstance(amt, (int, float)) and amt < 0:
                    self.add_issue(ValidationLevel.ERROR, f'{pfx}.amount', 'Cannot be negative')

        if not found_any:
            self.add_issue(ValidationLevel.INFO, 'funding', 'No funding rounds defined (bootstrapped?)')
    ...
    def validate_market_data(self):
        ...
        def sum_market(val):
            ...
            if isinstance(val, dict):
                ...
                if isinstance(streams, list) and streams:
                    return sum(s.get('value_m', 0) for s in streams if isinstance(s, dict))
                # Canonical: {regions: [{value_m: N}, ...]}
                regions = val.get('regions', [])
                if isinstance(regions, list) and regions:
                    return sum(r.get('value_m', 0) for r in regions if isinstance(r, dict))
        ...
        if tam is not None and tam < 0:
            self.add_issue(ValidationLevel.ERROR, 'tam', f'TAM cannot be negative: {tam}')
        if sam is not None and sam < 0:
            self.add_issue(ValidationLevel.ERROR, 'sam', f'SAM cannot be negative: {sam}')
        if som is not None and som < 0:
            self.add_issue(ValidationLevel.ERROR, 'som', f'SOM cannot be negative: {som}')

        # Logical hierarchy (only if all are in same units — skip if unclear)
        if all(v is not None for v in [tam, sam, som]):
            if sam > tam:
                self.add_issue(ValidationLevel.WARNING, 'market_data',
    ...
    def validate_customer_acquisition(self):
        ...
        if cac is not None:
            if not isinstance(cac, (int, float)):
                self.add_issue(ValidationLevel.ERROR, 'customer_acquisition.cac', 'Must be a number')
            elif cac < 0:
                self.add_issue(ValidationLevel.ERROR, 'customer_acquisition.cac', f'Cannot be negative: {cac}')
        ...
        if churn is not None:
            if not isinstance(churn, (int, float)):
                ...
                self.add_issue(ValidationLevel.ERROR, 'churn_rate',
                               f'Must be 0-1, got {churn}', 'Use 0.05 for 5%')
            elif churn > 0.20:
                self.add_issue(ValidationLevel.WARNING, 'churn_rate',
    ...
    def validate_consistency(self):
        ...
        if y0_revenue > 0 and total_y0_costs > 0:
            if total_y0_costs > y0_revenue * 5:
                self.add_issue(ValidationLevel.WARNING, 'consistency',
        ...
        if isinstance(rounds, list):
            total_funding = sum(r.get('amount', 0) for r in rounds if isinstance(r, dict))
    ...
    def print_results(self) -> bool:
        ...
        if not errors and not warnings:
            ...
            print(f'  Configuration has {len(warnings)} WARNING(S) — review before proceeding')
            print('=' * 60)
            return True
        else:
