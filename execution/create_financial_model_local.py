    ...
    try:
        ...
        try:
            ...
            for key, value in solution.items():
                ...
                val_str = str(val)
                if any(err in val_str for err in ['#REF!', '#VALUE!', '#DIV/0!', '#NAME?', '#N/A']):
...
def create_financial_model_local(company_name: str, config: dict = None, use_rapidtools: bool = False) -> str:
    ...
    for idx, stream in enumerate(config.get('revenue_streams', []), 1):
        ...
        for yr in range(11):
            ...
            ws.cell(row, col).value = f'=Assumptions!$B${assumptions_row}*Assumptions!{yr_col}${assumptions_row}'
            ws.cell(row, col).number_format = '#,##0'
        row += 1
    ...
    for yr in range(11):
        ...
        ws.cell(row, col).fill = PatternFill(start_color=Colors.GREEN, end_color=Colors.GREEN, fill_type='solid')
    total_rev_row = row
    ...
    ws.cell(row, 2, 'USD')
    avg_cogs = sum(s.get('cogs_pct', 0.30) for s in config.get('revenue_streams', [])) / max(len(config.get('revenue_streams', [])), 1)
    for yr in range(11):
        col = yr + 3
        col_letter = get_column_letter(col)
    ...
    for yr in range(11):
        col = yr + 3
        col_letter = get_column_letter(col)
        ws.cell(row, col).value = f'=IF({col_letter}{rev_row}=0,0,{col_letter}{gp_row}/{col_letter}{rev_row})'
    ...
    for yr in range(11):
        col = yr + 3
        col_letter = get_column_letter(col)
        ws.cell(row, col).value = f'=SUM({col_letter}{opex_start_row}:{col_letter}{opex_end_row})'
        ws.cell(row, col).number_format = '#,##0'
        ws.cell(row, col).font = Font(bold=True)
    total_opex_row = row
    row += 2
    ...
    for yr in range(11):
        col = yr + 3
        col_letter = get_column_letter(col)
        ws.cell(row, col).value = f'=IF({col_letter}{rev_row}=0,0,{col_letter}{ebitda_row}/{col_letter}{rev_row})'
        ws.cell(row, col).number_format = '0.0%'
