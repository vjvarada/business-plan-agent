...
def audit_balance_sheet(spreadsheet) -> List[AuditResult]:
    ...
    try:
        ...
        return [
            AuditResult(
                "Balance Sheet Access", AuditStatus.FAIL, f"Cannot access sheet: {e}"
            )
        ]
    ...
    if header_row is None:
        return [
            AuditResult(
                "Balance Sheet Structure", AuditStatus.FAIL, "Cannot find year headers"
            )
        ]
    ...
    for col_idx, year_name in year_cols:
        try:
            ...
            results.append(
                AuditResult(f"Balance Sheet {year_name}", AuditStatus.FAIL, str(e))
            )
...
def audit_valuations(spreadsheet, company_type: str = "saas") -> List[AuditResult]:
    ...
    if data is None:
        return [
            AuditResult(
                "Funding Sheet Access",
                AuditStatus.FAIL,
                "Funding sheet not found (tried: Funding & Cap Table, Funding Cap Table)",
            )
        ]
    ...
    try:
        ...
        if revenue_row and header_row:
            ...
            year_cols = [
                (j, h) for j, h in enumerate(headers) if h and h.startswith("Y")
            ]
    ...
    for i, row in enumerate(data):
        ...
        if post_money:
            ...
            year_key = (
                "Y0"
                if "seed" in first_cell.lower()
                else "Y2" if "series a" in first_cell.lower() else "Y3"
            )
    ...
    if not results:
        results.append(
            AuditResult(
                "Valuation Analysis",
                AuditStatus.WARN,
                "Could not analyze funding rounds",
            )
        )
...
def audit_metrics(spreadsheet) -> List[AuditResult]:
    ...
    if data is None:
        return [
            AuditResult(
                "Customer Economics Access",
                AuditStatus.FAIL,
                "Metrics sheet not found (tried: Customer Economics, Assumptions)",
            )
        ]
...
def print_audit_report(results: List[AuditResult], mode: str):
    ...
    for result in results:
        icon = (
            ...
            else "" if result.status == AuditStatus.WARN else ""
        )
