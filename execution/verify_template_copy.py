...
def verify_sheet_structure(spreadsheet) -> Dict[str, Any]:
    ...
    for i, expected in enumerate(EXPECTED_SHEETS):
        if i < len(actual_sheets) and actual_sheets[i] != expected:
            ...
            issues.append(('WARNING', f'Sheet order mismatch at position {i+1}: expected \'{expected}\', got \'{actual_sheets[i]}\''))
        ...
        for row_idx, row in enumerate(all_values, start=1):
            for col_idx in range(len(row)):
                try:
                    ...
                    cell_value = worksheet.acell(cell_addr, value_render_option='FORMULA').value
        ...
        for row_idx, row in enumerate(all_values, start=1):
            for col_idx, value in enumerate(row, start=1):
                if value and isinstance(value, str):
                    if any(err in value for err in ['#REF!', '#VALUE!', '#DIV/0!', '#N/A', '#ERROR!']):
...
def main():
    ...
    parser.add_argument('--detailed', action='store_true', help='Run detailed verification (slower)')
    ...
    # Exit codes
    errors = [i for i in (structure_result['issues'] + detail_issues) if i[0] == 'ERROR']
    warnings = [i for i in (structure_result['issues'] + detail_issues) if i[0] == 'WARNING']
